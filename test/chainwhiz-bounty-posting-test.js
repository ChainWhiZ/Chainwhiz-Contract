...
describe("ChainwhizCore Bounty Posting --> postIssue validations", function () {
  ...
  it("Should revert with error for setting reward less than threshold", async function () {
    const trxObj = chainwhiz.connect(a2).postIssue("efg", "www.google.com", tokensBN(1), tokensBN(2), 123456, 23456, 8910, { value: tokensBN(3) });
    expect(trxObj).to.be.revertedWith("POST_ISSUE_B")
    console.log("Should revert with error for setting reward less than threshold: Error in postIssue: Reawrd amount cannot be less than MIN_REWARD_AMOUNT")
  });

  it("Should revert with error for not transferring sufficient funds", async function () {
    const trxObj = chainwhiz.connect(a2).postIssue("efg", "www.google.com", tokensBN(10), tokensBN(2), 123456, 23456, 8910, { value: tokensBN(7) });
  ...
  it("Should successfully post an issue", async function () {
    expect(chainwhiz.connect(a2).postIssue("efg", "www.google.com", tokensBN(10), tokensBN(2), 123456, 23456, 8910, { value: tokensBN(12) })).to.be.emit(chainwhiz, "IssuePosted");
...
describe("ChainwhizCore Bounty Posting --> postIssue error validation", function () {
  ...
  beforeEach(async () => {
    ...
    chainwhiz = await Chainwhiz.connect(owner).deploy(owner.address)
    // await chainwhiz.connect(owner).initialize(owner.address);
    await chainwhiz.connect(a2).postIssue("efg", "www.google.com", tokensBN(10), tokensBN(2), 123456, 23456, 8910, { value: tokensBN(12) })
  })
  it("Should revert with error for linking same github id with different account for posting issue", async function () {
    const trxObj = chainwhiz.connect(a3).postIssue("efg", "www.google.com", tokensBN(10), tokensBN(2), 123456, 23456, 8910, { value: tokensBN(12) });
