...
describe("ChainwhizCore Bounty Posting --> postIssue validations", function () {
  ...
  it("Should revert with error for insufficient balance", async function () {
    try {
      ...
      const trxObj = chainwhiz.connect(a1).postIssue("abcd", "www.google.com", tokensBN(100), tokensBN(9000), 123456, 23456, 8910, "MATIC", { value: tokensBN(20000) });
      console.log(trxObj)
  ...
  it("Should revert with error for setting reward less than threshold", async function () {
    const trxObj = chainwhiz.connect(a2).postIssue("efg", "www.google.com", tokensBN(1), tokensBN(12), 123456, 23456, 8910, "MATIC", { value: tokensBN(3) });
  ...
  it("Should revert with error for setting community reward less than threshold", async function () {
    const trxObj = chainwhiz.connect(a2).postIssue("efg", "www.google.com", tokensBN(12), tokensBN(2), 123456, 23456, 8910, "MATIC", { value: tokensBN(14) });
  ...
  it("Should revert with error for not transferring sufficient funds", async function () {
    const trxObj = chainwhiz.connect(a2).postIssue("efg", "www.google.com", tokensBN(10), tokensBN(12), 123456, 23456, 8910, "MATIC", { value: tokensBN(7) });
...
describe("ChainwhizCore Bounty Posting --> postIssue error validation", function () {
  ...
  beforeEach(async () => {
    ...
    chainwhiz = await Chainwhiz.connect(owner).deploy(owner.address)
    // await chainwhiz.connect(owner).initialize(owner.address);
    await chainwhiz.connect(a2).postIssue("efg", "www.google.com", tokensBN(10), tokensBN(12), 123456, 23456, 8910, "MATIC", { value: tokensBN(22) })
  })
  it("Should revert with error for linking same github id with different account for posting issue", async function () {
    const trxObj = chainwhiz.connect(a3).postIssue("efg", "www.google.com", tokensBN(10), tokensBN(12), 123456, 23456, 8910, "MATIC", { value: tokensBN(22) });
