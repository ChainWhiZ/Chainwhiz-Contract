...
describe("ChainwhizCore Bounty Posting --> postIssue validations", function () {
  ...
  it("Should revert with error for insufficient balance", async function () {
    const trxObj = chainwhiz.connect(a1).postIssue("abcd","www.google.com", tokensBN(100),tokensBN(9000), 123456, 23456, 8910, true, { value: tokensBN(20000) });
    expect(trxObj).to.be.revertedWith("Error in postIssue: User doesnt have enough balance")
  });
  ...
  it("Should revert with error for setting reward less than threshold", async function () {
    ...
    expect(trxObj).to.be.revertedWith("Error in postIssue: Reawrd amount cannot be less than MIN_REWARD_AMOUNT")
  });
  ...
  it("Should revert with error for not transferring sufficient funds", async function () {
    ...
    expect(trxObj).to.be.revertedWith("Error in postIssue: User didnt transfer sufficient funds")
  });
  ...
  it("Should revert with error for linking same github id with different account for posting issue", async function () {
   ...
   expect(trxObj).to.be.revertedWith("Error in postIssue: The address linked github id is not the same");
  });
